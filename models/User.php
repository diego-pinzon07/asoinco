<?php
    class User{
        ...
        public function consultarUsuarios() {
            try {
                $userList = [];
                $sql = 'SELECT * FROM USUARIOS,ROLES WHERE usuarios.rol_codigo = roles.rol_codigo';
        ...
        public function consultarBeneficiarios() {
            try {
                $userList = [];
                $sql = 'SELECT roles.rol_nombre, usuarios.usuario_codigo, usuarios.usuario_nombre, usuarios.usuario_apellido, usuarios.usuario_correo, usuarios.usuario_estado
                FROM usuarios
                INNER JOIN roles ON roles.rol_codigo = usuarios.rol_codigo WHERE roles.rol_codigo = 6';
                $stmt = $this->dbh->query($sql);
                foreach ($stmt->fetchAll() as $user) {
                    $userList[] = new User(
                        $user['rol_nombre'],
        ...
        public function consultarAspirantes() {
            try {
                ...
                $sql = 'SELECT roles.rol_nombre, usuarios.usuario_codigo, usuarios.usuario_nombre, usuarios.usuario_apellido, usuarios.usuario_correo, usuarios.usuario_estado
                FROM usuarios
                INNER JOIN roles ON roles.rol_codigo = usuarios.rol_codigo WHERE roles.rol_codigo = 4';
                $stmt = $this->dbh->query($sql);
                foreach ($stmt->fetchAll() as $user) {
                    $userList[] = new User(
                        $user['rol_nombre'],
